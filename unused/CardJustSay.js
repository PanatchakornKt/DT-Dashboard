...
const CardJustSay = ({ input, justsay, setJustsay }) => {
  const onDeleteJustSay = (input) => {
    setJustsay(justsay.filter((justsays) => justsays.id !== input.id));
  };

  return (
    <>
      {justsay.map((justsays) => (
        <div
          key={justsays.id}
        ...
        >
          ...
          <div className="absolute top-5 right-5">
            ...
            <button
              ...
              onClick={() => onDeleteJustSay(justsays)}
            >
              <svg
                stroke="currentColor"
                fill="currentColor"
          ...
          <div className="text-center mt-8 mb-12">
            <h1 className="text-4xl font-bold undefined">{justsays.input}</h1>
          </div>
